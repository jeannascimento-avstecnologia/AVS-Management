...
export type TifluxBillingClient = {
  id: number
  name: string
  cnpj: string | null
}
...
type Props = {
  ...
  onSelect: (client: TifluxBillingClient) => void
}

export function TifluxBillingClientSearch({
  value,
  disabled,
  placeholder = 'Buscar cliente TiFlux (nome ou CNPJ)…',
  onChange,
  onSelect,
}: Props) {
  const listId = useId()
  ...
  const query = useQuery({
    queryKey: ['tiflux-billing-clients', debounced],
    queryFn: () => api.searchTifluxBillingClients(debounced, 20),
